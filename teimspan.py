...
class Addspan(object):
    ...
    def set_id_to_span_data(self, nd_data):
        try:
            ...
            loginfo.log("ERROR! teimspam set_id_to_span_data()")
            loginfo.log(str(e))
            d = nd_data
    ...
    def find_span_data(self):
        ...
        for nd in root.iter():
            ...
            if tag in ['w', 'l']:
                ...
                if val.find(OPMON) > -1:
                    ...
                    self.spam_from_src_xml(nd_data, 'monologue')
                    continue
                # dicorso diretto
                if val.find(OPDD) > -1:
                    self.write_span_log('D', nd)
                    self.spam_from_src_xml(nd_data, 'directspeech')
                    continue
                if val.find(CLDM) > -1:
                    if text.strip() == CLDM:
        ...
        self.xml_txt = xml.replace(OPDD, '').replace(OPMON, '').replace(CLDM, '')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        parser.print_help()
        sys.exit()
    parser.add_argument('-t', dest="tag", required=False, default=None, metavar="", help="[-t <file tags span>] ")
